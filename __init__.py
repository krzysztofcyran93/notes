...
def create_app(test_config=None):
    ...
    from .models import db, User, Note, Role, UserRoles
    ...
    def assign_role(methods=('POST')):
        admin_role = Role(name='Admin')
        db.session.commit()
        if not User.query.filter_by(User.username=='kris').first():
            user = User(username='kris', password=user_manager.hash_password('Password1'))
            db.session.add(user)
            db.session.commit()
